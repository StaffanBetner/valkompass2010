...
load("valkompass2010/Sweden2010.Rda")
...
candidates <- c(output, as_tibble(survey_blackbox$individuals[[2]])) %>% as_tibble
...
candidates[candidates[, 56:57] %>% duplicated() %>% which(), ] <-
  candidates[candidates[, 56:57] %>% duplicated() %>% which(), ] %>% 
  mutate(c1 = jitter(c1, factor = 0.1, 0.1), 
         c2 = jitter(c2, factor = 0.1, 0.1))
