...
library(plotly)
load(".RData")
xaxis = list(range = c(-4.2, 3.4))
yaxis = list(range = c(-3.7, 4.5))

shinyServer(function(input, output) {
  output$plot <- renderPlotly({
    if (input$fargsattning == "Parti") {
      parti_obj <-
        joined_dataset %>% dplyr::filter(vald %in% input$vilka) %>% dplyr::filter(Parti %in% input$vilka2) %>%
        ggplot(aes(
          x = F1,
          y = F2,
          name = Namn,
  ...
  output$plot_parti <-
    renderPlotly({
      parti_means <-
        joined_dataset %>% dplyr::filter(vald %in% input$vilka_parti) %>% dplyr::filter(Parti %in% input$vilka2_parti) %>%
        ...
        if (input$vad == 0) {
          geom_point(
            aes(x = F1, y = F2),
            data = (
              joined_dataset %>% dplyr::filter(vald %in% input$vilka_parti) %>%
                group_by(Parti) %>%
                dplyr::summarise(F1 = mean(F1, na.rm=T),
                          F2 = mean(F2, na.rm=T)) %>% dplyr::filter(Parti %in% input$vilka2_parti)
            )
          )
        } else{
          geom_point()
